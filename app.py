    ...
    for c in enriched:
        ...
        if dist<=radius:
            out.append({
                ...
                "lat":c["lat"],"lon":c["lon"],"distance":dist,"status":"Scored",
                "score":score,"parts":parts,"record":c,"source_type":"Enriched"
            })
    return out
...
def chamber_for_center(center_name, radius):
    ...
    for c in chamber_cache.get(center_name,[]):
        ...
        if dist<=radius:
            out.append({
                "company":c["company"],"industry":"Chamber-listed business","city":c.get("address",""),
                "lat":lat,"lon":lon,"distance":dist,"status":"Needs enrichment",
                "score":None,"parts":None,"record":c,"source_type":"Chamber lead"
            })
    return out
...
if page=="Discovery":
    ...
    with c3:
        view_mode=st.selectbox("Records",["All organizations","Scored only","Needs enrichment"])
    with c4:
        min_score=st.selectbox("Minimum scored fit",[0,50,60,70,80,90],format_func=lambda x:"Any score" if x==0 else f"{x}+")
    center=CENTERS[center_name]

    scored=enriched_for_center(center_name,radius)
    leads=chamber_for_center(center_name,radius)

    scored=[r for r in scored if r["score"]>=min_score]
    if view_mode=="Scored only":
        rows=scored
    elif view_mode=="Needs enrichment":
        rows=leads
    else:
        rows=scored+leads
    ...
    with right:
        ...
        st.caption("Click a score or Open button to view the organization.")
        if "selected_company" not in st.session_state and rows:
            st.session_state.selected_company=rows[0]["company"]
        for r in rows[:40]:
            a,b=st.columns([4.2,1.1])
            with a:
                st.markdown(f"**{r['company']}**")
                st.caption(f"{r['status']} · {r['distance']} mi")
            with b:
                label=str(r["score"]) if r["score"] is not None else "Open"
                if st.button(label,key="open_"+r["company"],use_container_width=True):
                    st.session_state.selected_company=r["company"]
                    st.rerun()
    ...
    if rows:
        ...
        with top1:
            if selected["status"]=="Scored":
                badges=f'<span class="badge green">Evidence scored</span><span class="badge">{rec.get("capacity_tier","").title()} capacity</span>'
                summary=rec.get("summary","")
                source=rec.get("discovery_source","Direct company research")
            else:
                badges=f'<span class="badge amber">Needs enrichment</span><span class="badge">{rec.get("discovery_source","Chamber directory")}</span>'
                summary="This organization was discovered from a nearby Chamber / business-alliance directory. Philanthropy, STEM, recruiting, and sponsorship evidence has not yet been fully verified."
                source=rec.get("discovery_source","Chamber directory")
            st.markdown(f"""<div class="card"><div class="name">{selected['company']}</div>
            <div class="muted">{rec.get('address') or selected.get('city','')} · {selected['distance']} miles from {center_name}</div>
            {badges}
            <p style="margin-top:14px">{summary}</p>
            <div class="small"><b>Discovered via:</b> {source}</div></div>""",unsafe_allow_html=True)
        ...
        with top2:
            if selected["score"] is not None:
                ...
                <b>{label}</b><div class="small" style="margin-top:7px">Evidence-backed weighted score</div></div>""",unsafe_allow_html=True)
            else:
                st.markdown("""<div class="card" style="text-align:center">
                <div class="small">SPONSOR SCORE</div><div class="score">—</div>
                <b>Enrichment pending</b><div class="small" style="margin-top:7px">No score until enough evidence is verified</div></div>""",unsafe_allow_html=True)

        # Contact actions
        website=rec.get("website","")
        contact=rec.get("contact_page","")
        email=rec.get("contact_email","")
        profile=rec.get("profile_url","")
        a,b,c=st.columns(3)
        with a:
            if website:
        ...
        with c:
            if profile:
                ...
                st.button("Source in evidence ledger",disabled=True,use_container_width=True)
        ...
        if selected["score"] is not None:
            ...
            st.info("This lead will receive a sponsor score only after company-site evidence is collected and reviewed.")
    ...
    st.markdown("The score is computed from structured evidence and organization metadata. **No LLM generates the final score.**")
    ...
    for c in enriched:
        for s in c["sources"]:
            records.append({"Organization":c["company"],"Type":"Scoring evidence","Source":s["title"],
                            "Official":s["official"],"Date":s.get("date") or "Undated","URL":s["url"]})
    for area,items in chamber_cache.items():
        for c in items:
            records.append({"Organization":c["company"],"Type":"Chamber discovery","Source":c.get("discovery_source",""),
                            "Official":False,"Date":"Current directory","URL":c.get("profile_url","")})
