...
def scored_rows(center_name, radius):
    ...
    for company in enriched:
        ...
        if distance <= radius:
            rows.append({"company": company["company"], "lat": company["lat"], "lon": company["lon"], "distance": distance, "score": score, "parts": parts, "record": company, "status": "analyzed"})
    return rows
...
def chamber_rows(center_name, radius):
    ...
    for company in chamber.get(center_name, []):
        ...
        if distance <= radius:
            rows.append({"company": company["company"], "lat": company["lat"], "lon": company["lon"], "distance": distance, "score": None, "parts": None, "record": company, "status": "local"})
    return rows
...
if page == "Discovery":
    ...
    center = CENTERS[center_name]
    analyzed = [row for row in scored_rows(center_name, radius) if row["score"] >= minimum]
    ...
    with left:
        ...
        if rows:
            ...
            layer = pdk.Layer("ScatterplotLayer", map_df, get_position="[lon,lat]", get_radius=3, radius_units="pixels", radius_scale=1, radius_min_pixels=2, radius_max_pixels=3, get_fill_color=[0,103,185,190], pickable=True, auto_highlight=True, stroked=True, get_line_color=[255,255,255,160], line_width_min_pixels=0.5)
        ...
        if rows:
            list_df = pd.DataFrame([{"Company":r["company"],"Miles":r["distance"],"Score":r["record"].get("ai_analysis",{}).get("ai_score",r["score"] if r["score"] is not None else None),"Website":company_website(r["record"])} for r in rows])
            event = st.dataframe(list_df, use_container_width=True, hide_index=True, height=330, selection_mode="single-row", on_select="rerun", column_config={"Miles":st.column_config.NumberColumn("Miles",format="%.1f"),"Score":st.column_config.NumberColumn("Score",format="%d"),"Website":st.column_config.LinkColumn("Website",display_text="Open")})
    ...
    with right:
        ...
        if not selected: st.info("Select a company from the list.")
        else:
            record = selected["record"]; ai = record.get("ai_analysis"); nc = national_relationship(selected["company"]); website = company_website(record); profile = safe_profile_url(record); contact = record.get("contact_page",""); email = record.get("contact_email","")
            ...
            badge_html = ('<span class="badge green">SHPE National Contact</span>' if nc else '') + ('<span class="badge green">AI analyzed</span>' if ai else '')
            ...
            if score_value is not None:
                s1,s2=st.columns([1,1.3]); s1.metric("Sponsor score",f"{score_value}/100"); s2.metric("Assessment",score_label)
            else: st.metric("Sponsor score","Research in progress")
            ...
            with tab1:
                if nc:
                    ...
                    if is_public_url(source_url): st.link_button("View SHPE source",source_url,use_container_width=True)
                cweb,cemail=st.columns(2)
                with cweb:
                    if website: st.link_button("Visit company website",website,use_container_width=True)
                    elif profile: st.link_button("View Chamber listing",profile,use_container_width=True)
                    else: st.button("Company website unavailable",disabled=True,use_container_width=True)
                with cemail:
                    ...
                    else: st.button("Public contact unavailable",disabled=True,use_container_width=True)
            ...
            with tab2:
                if ai:
                    ...
                    st.write(f"**Suggested ask:** {ai['recommended_ask']}"); st.write(f"**Outreach angle:** {ai['outreach_angle']}"); st.write(ai.get("summary",""))
                    if ai.get("strengths"):
                ...
                else: st.info("Sponsorship analysis is being expanded for this company.")
            ...
            with tab3:
                ...
                else: st.info("Detailed sponsorship metrics are not yet available for this company.")
    ...
    st.subheader("Evidence sources"); records=[]
    for company in enriched:
        for source in display_evidence(company): records.append({"Organization":company["company"],"Source":source["title"],"URL":source["url"]})
